...
contract FeesTest is SendEarnTest {
    ...
    function _feeShares() internal view returns (uint256) {
        ...
        uint256 feeShares =
            sendFeeAssets.mulDiv(sevault.totalSupply() + 1, totalAssetsAfter - sendFeeAssets + 1, Math.Rounding.Floor);
        return feeShares;
    }
    ...
    function testAccrueFeeWithinABlock(uint256 deposited, uint256 withdrawn) public {
        ...
        assertApproxEqAbs(sevault.balanceOf(SEND_FEE_RECIPIENT), 0, 1, "sevault.balanceOf(SEND_FEE_RECIPIENT)");
    }

    function testDepositeAccrueFee(uint256 deposited, uint256 newDeposit, uint256 blocks) public {
        ...
        assertEqLastTotalAssets("lastTotalAssets2");
        assertEq(sevault.balanceOf(SEND_FEE_RECIPIENT), feeShares, "sevault.balanceOf(SEND_FEE_RECIPIENT)");
    }

    function assertEqLastTotalAssets(string memory err) internal {
        assertApproxEqAbs(sevault.lastTotalAssets(), sevault.totalAssets(), 1, string.concat("sevault.", err));
        assertApproxEqAbs(vault.lastTotalAssets(), vault.totalAssets(), 1, string.concat("vault.", err));
